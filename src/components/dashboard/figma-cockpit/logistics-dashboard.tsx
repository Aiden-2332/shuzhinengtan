...
import {
  ...
  Bar, CartesianGrid, ComposedChart, XAxis, YAxis,
  LineChart, Line,
  Legend,
} from 'recharts'
import { useRealtimeNow } from '@/hooks/use-realtime-now';
import { getRealtimeLoadData } from '@/data/operations-data';
...
export function LogisticsBottom() {
  ...
  const hourlyData = useMemo(() => {
    // Keep the server render deterministic, then swap to the live campus hour
    // after hydration without maintaining a second chart-specific dataset.
    const anchor = nowMs === null ? new Date('2026-08-06T05:00:00.000Z') : new Date(nowMs);
    return getRealtimeLoadData(anchor).map((item) => ({
      hour: item.time,
  ...
  return (
    <div className="glass-section" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div className="glass-title">校园实时负荷</div>
      <div style={{ flex: 1, padding: '4px 8px 4px 0' }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={hourlyData} margin={{ top: 6, right: 12, bottom: 2, left: -12 }}>
            ...
            <Tooltip {...TS} formatter={(value, name) => [`${Number(value).toLocaleString()} kW`, name === 'realtime' ? '实时' : name === 'forecast' ? '预测' : '昨日']} />
            <Legend
              verticalAlign="top"
              align="left"
              height={18}
              content={() => (
                <div style={{ display: 'flex', gap: 12, alignItems: 'center', paddingLeft: 8, color: 'rgba(160,210,255,.72)', fontSize: 9 }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><i style={{ width: 14, height: 2, background: CYAN, display: 'inline-block' }} />实时</span>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><i style={{ width: 14, height: 7, background: BLUE, opacity: .72, display: 'inline-block' }} />昨日</span>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><i style={{ width: 14, height: 2, background: GREEN, display: 'inline-block' }} />预测</span>
                </div>
              )}
            />
            {/* Bars are declared first so both target-aligned lines stay in front. */}
            <Bar dataKey="yesterday" name="昨日" fill={BLUE} fillOpacity={0.72} stroke={BLUE} strokeOpacity={0.52} maxBarSize={18} radius={[1, 1, 0, 0]} isAnimationActive={false} />
            <Line type="monotone" dataKey="forecast" name="预测" stroke={GREEN} strokeWidth={1.8} strokeDasharray="6 3" dot={false} activeDot={{ r: 3 }} isAnimationActive={false} />
            <Line type="monotone" dataKey="realtime" name="实时" stroke={CYAN} strokeWidth={2.2} dot={false} activeDot={{ r: 3 }} isAnimationActive={false} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </div>

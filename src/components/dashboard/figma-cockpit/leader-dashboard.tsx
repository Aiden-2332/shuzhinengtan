...
import {
  ...
  Bar, ComposedChart, Line, XAxis, YAxis, CartesianGrid, Legend,
} from 'recharts'
...
export function LeaderBottom() {
  ...
  const monthlyTrend = useMemo(() => {
    // Keep the server render deterministic, then use the shared campus clock after hydration.
    const anchor = nowMs === null ? new Date('2026-08-06T05:00:00.000Z') : new Date(nowMs)
    return getMonthlyTrendData(anchor)
  }, [nowMs])

  return (
    <div className="glass-section leader-trend-section" style={{ height: '100%', display: 'flex', flexDirection: 'column' }}>
      <div className="glass-title">近12个月月度排放</div>
      <div
        role="img"
        aria-label="近12个月月度排放，包含实际、目标和预测数据"
        className="leader-trend-plot"
      >
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={monthlyTrend} margin={{ top: 22, right: 12, bottom: 2, left: -12 }}>
            <CartesianGrid strokeDasharray="3 6" stroke="rgba(137,196,222,.14)" />
            <XAxis dataKey="month" tick={{ fontSize: 10, fill: 'rgba(120,175,225,.7)' }} tickLine={false} axisLine={{ stroke: 'rgba(0,100,160,.2)' }} />
            <YAxis domain={[0, 'auto']} tick={{ fontSize: 10, fill: 'rgba(148,194,216,.82)' }} tickLine={false} axisLine={false} />
            <Tooltip
              {...TS}
              formatter={(value, name) => [
                `${Number(value).toLocaleString('zh-CN')} tCO₂e`,
                name === 'actual' ? '实际' : name === 'target' ? '目标' : '预测',
              ]}
            />
            <Legend
              verticalAlign="top"
              ...
              height={18}
              content={() => (
                <div style={{ display: 'flex', gap: 12, alignItems: 'center', paddingLeft: 8, color: 'rgba(188,221,237,.82)', fontSize: 9 }}>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><i style={{ width: 10, height: 6, background: CYAN, display: 'inline-block', opacity: .82 }} />实际</span>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><i style={{ width: 14, borderTop: `2px dashed ${YELLOW}`, display: 'inline-block' }} />目标</span>
                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><i style={{ width: 14, borderTop: `2px dotted ${RED}`, display: 'inline-block' }} />预测</span>
                </div>
              )}
            />
            <Bar dataKey="actual" name="实际" fill={CYAN} fillOpacity={0.8} stroke={CYAN} strokeOpacity={0.65} maxBarSize={18} radius={[2, 2, 0, 0]} isAnimationActive={false} />
            <Line type="monotone" dataKey="target" name="目标" stroke={YELLOW} strokeWidth={1.8} strokeDasharray="8 4" dot={{ r: 2, fill: '#071b2a', stroke: YELLOW, strokeWidth: 1.2 }} activeDot={{ r: 3.5, fill: YELLOW, stroke: '#071b2a', strokeWidth: 1.5 }} isAnimationActive={false} />
            <Line type="monotone" dataKey="forecast" name="预测" stroke={RED} strokeWidth={1.8} strokeDasharray="3 4" dot={{ r: 2, fill: RED, stroke: '#071b2a', strokeWidth: 1 }} activeDot={{ r: 3.5, fill: RED, stroke: '#071b2a', strokeWidth: 1.5 }} isAnimationActive={false} />
          </ComposedChart>
        </ResponsiveContainer>
      </div>
    </div>

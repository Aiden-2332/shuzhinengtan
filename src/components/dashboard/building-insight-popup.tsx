    ...
    <>
      <div className="px-4 py-3">
        <div className="flex items-end justify-between gap-4">
          <div>
            <div className="text-xs text-slate-300">预计排放相对年度限额</div>
        ...
        <div className="mt-3 grid grid-cols-2 border-y border-white/10 text-[13px] sm:grid-cols-3">
      ...
      <section className="px-4 pb-3" aria-label="过去十二个月相对限额比例">
        <div className="mb-1 flex items-center justify-between gap-3">
          ...
          <span className="text-xs text-slate-400">100% 为限额线</span>
        </div>
      ...
      <section className="border-t border-white/10 px-4 py-3" aria-labelledby={`peer-benchmark-${building.id}`}>
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-2">
            ...
            <h3 id={`peer-benchmark-${building.id}`} className="truncate text-sm font-semibold text-slate-200">同类楼宇横向对标</h3>
          </div>
          <span className="shrink-0 text-xs text-slate-400">{benchmark.peerLabel} · {benchmark.peerCount} 栋</span>
        </div>

        <div className="mt-3 grid grid-cols-2 border-y border-white/10 text-[13px]">
          <div className="py-2.5 pr-3">
            <div className="text-slate-500">能耗强度</div>
            ...
            <p className="mt-1 text-xs leading-5 text-slate-400">同类中位 {formatNumber(benchmark.energyMedian, 1)} · 高耗能序位 {benchmark.energyRank}/{benchmark.peerCount}</p>
          </div>
          <div className="border-l border-white/10 px-3 py-2.5">
            ...
            <p className="mt-1 text-xs leading-5 text-slate-400">kgCO₂e/㎡ · 高碳序位 {benchmark.carbonRank}/{benchmark.peerCount}</p>
          </div>
        </div>

        <div className="mt-2.5 flex items-start justify-between gap-3 text-xs leading-5">
      ...
      <dl className="grid grid-cols-2 gap-x-4 gap-y-2 border-t border-white/10 px-4 py-3 text-[13px] leading-5">
      ...
      <div className="sticky bottom-0 z-10 border-t border-white/10 bg-[#071923]/96 p-3 backdrop-blur-md">
        <Link
          ...
          className="flex h-10 w-full items-center justify-center gap-2 rounded-md bg-cyan-300 px-3 text-sm font-semibold text-cyan-950 transition-colors hover:bg-cyan-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
        >
  ...
  return (
    <>
      <div className="flex items-center justify-between gap-4 border-b border-white/10 px-4 py-2.5">
        <span className="text-[13px] text-slate-300">当前未关闭问题</span>
        <span className={`rounded-full px-2.5 py-1 text-[13px] font-semibold ${issues.length ? "bg-rose-500/15 text-rose-200" : "bg-emerald-500/15 text-emerald-200"}`}>
          {issues.length} 项
        </span>
      </div>
        ...
        <section className="border-b border-white/10 bg-emerald-400/6 px-4 py-3" aria-label="已定位历史告警">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0">
              ...
              <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-xs leading-5 text-slate-300">
            ...
            <span className="shrink-0 rounded-full bg-emerald-500/15 px-2 py-0.5 text-xs font-medium text-emerald-200">
              {focusedHistoricalAlarm.status === "resolved" ? "已解决" : "已关闭"}
            </span>
          </div>
          <p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-300">{focusedHistoricalAlarm.description}</p>
          <Link
            href={`/alarms?alarm=${encodeURIComponent(focusedHistoricalAlarm.id)}`}
            className="mt-2 inline-flex items-center gap-1 text-xs font-medium text-cyan-300 hover:text-cyan-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300"
          >
      ...
      {issues.length ? (
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="truncate text-[13px] font-semibold text-slate-100">{alarm.title}</h3>
                    </div>
                    <div className="mt-1.5 flex flex-wrap gap-x-3 gap-y-1 text-xs leading-5 text-slate-300">
                  ...
                  <span className={`shrink-0 rounded-full px-2 py-0.5 text-xs font-medium ${SEVERITY_META[alarm.severity].bgColor} ${SEVERITY_META[alarm.severity].color}`}>
                    {SEVERITY_META[alarm.severity].label}
                  </span>
                </div>
                <p className="mt-2 line-clamp-2 text-xs leading-5 text-slate-300">{alarm.description}</p>
        ...
        <div className="flex min-h-32 flex-col items-center justify-center px-6 text-center">
          ...
          <p className="mt-1 text-xs leading-5 text-slate-400">设备、能源、环境和数据状态均无未关闭告警</p>
        </div>
      )}
      ...
      <div className="sticky bottom-0 z-10 border-t border-white/10 bg-[#071923]/96 p-3 backdrop-blur-md">
        <Link
          ...
          className="flex h-10 w-full items-center justify-center gap-2 rounded-md border border-cyan-300/30 bg-cyan-300/10 px-3 text-sm font-semibold text-cyan-100 transition-colors hover:bg-cyan-300/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300"
        >
          打开告警中心
          <ArrowUpRight aria-hidden="true" className="h-3.5 w-3.5" />
        </Link>
...
export function BuildingInsightPopup({
  ...
  return (
    <div
      className="w-[min(420px,calc(100vw-24px))] max-h-[min(620px,78vh)] overscroll-contain overflow-y-auto rounded-lg border border-cyan-300/25 bg-[#071923]/96 text-slate-100 shadow-[0_18px_50px_rgba(0,0,0,.48)] backdrop-blur-xl"

  ...
  return (
    <main className="gateway-shell gateway-placeholder-shell">
      ...
      <section className="gateway-placeholder">
        ...
        <p>PLATFORM ACCESS</p>
        <h1>{name}</h1>
        <div className="gateway-placeholder-status"><span /> 外部系统链接待接入</div>
        <p className="gateway-placeholder-copy">入口已经建立。收到正式链接后，可直接替换跳转地址，无需调整门户页布局。</p>
        <Link href="/gateway"><ArrowLeft /> 返回统一门户</Link>
        <span className="gateway-placeholder-hint"><ExternalLink /> 此页面用于验证入口可点击</span>
      </section>
    </main>
  );
}

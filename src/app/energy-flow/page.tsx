...
const selectClass = "h-9 w-full rounded-md border border-[#28505c] bg-[#0d202c] px-2.5 text-xs text-[#f2fbfc] outline-none transition focus:border-[#35d4e4] focus:ring-2 focus:ring-[#35d4e4]/20";
const inputClass = "h-9 w-full rounded-md border border-[#28505c] bg-[#0d202c] px-2.5 text-xs text-[#f2fbfc] outline-none transition focus:border-[#35d4e4] focus:ring-2 focus:ring-[#35d4e4]/20";
const cardClass = "rounded-xl border border-[#28505c] bg-[#0d202c] shadow-[0_14px_32px_rgba(0,9,17,.2)]";
          ...
          <section className="grid items-stretch gap-4 xl:grid-cols-[minmax(0,7fr)_minmax(330px,3fr)]">
              ...
              <div className="border-b border-slate-100 px-4 py-3">
                ...
                {isFullscreen && (
                  <div className="mt-3 grid grid-cols-3 gap-2 rounded-lg bg-slate-50 p-2 text-xs">
                    ...
                    <div><span className="text-slate-400">综合利用率</span><b className="ml-2 text-cyan-200">{kpis.efficiency.toFixed(1)}%</b></div>
                  </div>
                )}
            ...
            <aside className={cn(cardClass, "flex min-h-[650px] min-w-0 flex-col overflow-hidden")}>
              <Tabs value={rightTab} onValueChange={(value) => setRightTab(value as RightTab)} className="h-full gap-0">
                ...
                <TabsContent value="efficiency" className="m-0 p-4"><div className="rounded-xl bg-[#123340] p-4"><Gauge className="h-7 w-7 text-cyan-300" /><h3 className="mt-3 font-semibold">效率分析</h3><div className="mt-2 text-3xl font-bold text-cyan-200">{kpis.efficiency.toFixed(1)}%</div><p className="mt-2 text-xs leading-5 text-slate-600">当前综合能源利用率距年度目标仍差 {(90 - kpis.efficiency).toFixed(1)} 个百分点。主要改善空间来自变配电损耗、空调部分负荷运行和夜间待机。</p><Button size="sm" className="mt-4" onClick={() => openDiagnosis(selectedNode)}><Wrench />进入能源诊断</Button></div></TabsContent>
              </Tabs>
            </aside>
          </section>
        ...
        {node && <div className="flex-1 space-y-4 overflow-y-auto p-5">
          ...
          <section className="rounded-lg border border-slate-100 p-3">
            ...
            <div className="h-150px h-[150px]">
              <ResponsiveContainer width="100%" height="100%"><AreaChart data={TREND_DATA.map((item) => ({ ...item, value: item.input * Math.max(0.08, node.standardCoal / 189.6) * scale }))}><defs><linearGradient id="nodeTrend" x1="0" y1="0" x2="0" y2="1"><stop offset="5%" stopColor="#2563eb" stopOpacity={0.25} /><stop offset="95%" stopColor="#2563eb" stopOpacity={0} /></linearGradient></defs><CartesianGrid strokeDasharray="3 3" stroke="#2b4f5b" /><XAxis dataKey="time" tick={{ fontSize: 9 }} interval={5} /><YAxis tick={{ fontSize: 9 }} width={34} /><Tooltip /><Area type="monotone" dataKey="value" name="能源量" stroke="#2563eb" fill="url(#nodeTrend)" /></AreaChart></ResponsiveContainer>
            </div>
          </section>
          <section className="rounded-lg border border-slate-100 p-3">
            ...
            <div className="h-[130px]"><ResponsiveContainer width="100%" height="100%"><LineChart data={sevenDays}><CartesianGrid strokeDasharray="3 3" stroke="#2b4f5b" /><XAxis dataKey="day" tick={{ fontSize: 9 }} /><YAxis tick={{ fontSize: 9 }} width={34} /><Tooltip /><Line type="monotone" dataKey="value" name="能源量" stroke="#06b6d4" strokeWidth={2} dot={{ r: 2 }} /></LineChart></ResponsiveContainer></div>
          </section>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
        ...
        {link && <>
          ...
          <section className="rounded-lg border border-slate-100 p-3"><div className="mb-2 flex justify-between"><h4 className="text-xs font-semibold">24小时趋势</h4><span className="text-[10px] text-slate-400">点击异常时段可在主图定位</span></div><div className="h-[190px]"><ResponsiveContainer width="100%" height="100%"><LineChart data={values}><CartesianGrid strokeDasharray="3 3" stroke="#2b4f5b" /><XAxis dataKey="time" tick={{ fontSize: 9 }} interval={3} /><YAxis tick={{ fontSize: 9 }} width={38} /><Tooltip /><Line type="monotone" dataKey="value" name="流量" stroke={ENERGY_META[link.energyType].color} strokeWidth={2} dot={(props: { cx?: number; cy?: number; payload?: { abnormal?: boolean } }) => <circle cx={props.cx} cy={props.cy} r={props.payload?.abnormal ? 5 : 2} fill={props.payload?.abnormal ? "#ef4444" : ENERGY_META[link.energyType].color} /> } /></LineChart></ResponsiveContainer></div></section>
